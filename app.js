...
function focusMapCamera(region) {
  ...
  const translateX = (mapWidth / 2 - focusPointX) * (cameraFocusScale - 1) + mapWidth * .16;
  const translateY = (mapHeight / 2 - focusPointY) * (cameraFocusScale - 1) - mapHeight * .05;
...
function showOverview() {
  ...
  document.getElementById('region-name').textContent = '业务分布';
  document.getElementById('region-category').textContent = '业务分布';
