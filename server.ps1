...
while ($true) {
  ...
  try {
    ...
    $allowedPaths = @('/index.html', '/styles.css', '/app.js', '/three-map.js', '/data/china.geojson', '/data/background.mp4', '/assets/company-logo-strip.jpg')

    if ($allowedPaths -contains $requestPath) {
      $filePath = Join-Path $root $requestPath.TrimStart('/').Replace('/', '\')
      $body = [System.IO.File]::ReadAllBytes($filePath)
      $contentType = if ($requestPath.EndsWith('.css')) { 'text/css; charset=utf-8' } elseif ($requestPath.EndsWith('.js')) { 'application/javascript; charset=utf-8' } elseif ($requestPath.EndsWith('.geojson')) { 'application/geo+json; charset=utf-8' } elseif ($requestPath.EndsWith('.mp4')) { 'video/mp4' } elseif ($requestPath.EndsWith('.jpg')) { 'image/jpeg' } else { 'text/html; charset=utf-8' }
      $status = '200 OK'
    } else {
